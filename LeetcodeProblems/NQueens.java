...
public class NQueens {
    public static List<List<String>> solveNQueens(int n) {
        ...
        QueenPlacement(list, chess, 0, n, 0, n);
        return list;
    }
    ...
    public static void Naive(List<List<String>> list, int[][] chess, int qpsf, int tq, int row, int col, int n){
        //Base
        if(qpsf == tq){
            //chess[row][col] = 1;
            List<String> currAns = StrConv(chess, n);
        ...
        if(isQueenSafe(chess, row, col, n)){
            chess[row][col] = 1;
            Naive(list, chess, qpsf+1, tq, row, col+1, n);
            chess[row][col] = 0;
        }
        Naive(list, chess, qpsf, tq, row, col+1, n);
    }
